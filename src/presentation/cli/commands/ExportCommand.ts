...
import { ExportTasksUseCase, type ExportFormat, type ExportOptions } from '../../../application';
...
export class ExportCommand {
  static create(repository: ITaskRepository): Command {
    ...
    return new Command('export')
      ...
      .action(async (options: { format: string; output?: string; pretty?: boolean }) => {
        try {
          ...
          const filename = options.output || `tasks-${timestamp}.${options.format}`;
          const filepath = path.resolve(filename);

          fs.writeFileSync(filepath, data);
          Logger.success(`Tasks exported to ${filepath}`);
